...
def train_gp_models(training_data, pruned_metrics_idxs):
...
def main(run_id):
    ...
    models, scalers = train_gp_models(data['training'], pruned_metrics_idxs)
    ...
    for test_job in proxy_jobs.keys():
        ...
        errors[test_job], training_errors[test_job] = train_and_evaluate(
            test_job, proxy_job, data['training'], data['observed'], data['test'], obj_idx)
    ...
    for test_job in errors.keys():
        if test_job in ML_TEST_JOBS:
            ml_errs.append(errors[test_job])
        else:
            sql_errs.append(errors[test_job])
    ...
    logging.info("Avg Errors across ML jobs: {:.2f}%".format(np.mean(ml_errs)))
    logging.info("Avg Errors across SQL jobs: {:.2f}%".format(
        np.mean(sql_errs)))
...
if __name__ == "__main__":
    # FIXME: this is a quick way to implement this... fix this later on by running some parallel threads...
    runs_data = {'runs': {},
                 'CONFIG': CONFIG}
    for i in range(N_RUNS):
        runs_data['runs'][i] = main(i)
    np.save(OUTPUT_FNAME, runs_data)
